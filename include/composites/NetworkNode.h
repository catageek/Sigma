...
namespace Sigma {
	...
	public:
		...
		static std::vector<std::unique_ptr<IECSComponent>> AddEntity(id_t id, network::TCPConnection&& cnx) {
		...
		static void RemoveEntity(id_t id) {
			if(connection_map.count(id)) {
		...
		static int GetFileDescriptor(id_t id) {
			if(fd_map.count(id)) {
				return fd_map.at(id);
			}
			return -1;
